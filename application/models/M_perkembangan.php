...
class M_perkembangan extends CI_Model
{
	...
	{
		$query = 'SELECT * FROM users s,baby b, perkembangan p INNER JOIN (SELECT max(tanggal) as max_tanggal FROM perkembangan GROUP by id_bayi) p1 on p.tanggal = p1.max_tanggal where tanggal IN (SELECT max(tanggal) FROM perkembangan where p.id_bayi = id_bayi) and s.id = b.id_user and b.id_baby = p.id_bayi GROUP BY id_bayi';
		return $this->db->query($query)->result();
	}
	public function read_per_user($id)
	{
		$query = 'SELECT * FROM users s,baby b, perkembangan p INNER JOIN (SELECT max(tanggal) as max_tanggal FROM perkembangan GROUP by id_bayi) p1 on p.tanggal = p1.max_tanggal where tanggal IN (SELECT max(tanggal) FROM perkembangan where p.id_bayi = id_bayi) and b.id_baby = p.id_bayi and s.id = b.id_user and s.id = "' . $id . '" group by id_bayi';
		return $this->db->query($query)->result();
	}
	...
	{
		return $this->db->insert($this->_table_perkembangan, $data);
	}
